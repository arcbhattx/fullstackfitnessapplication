...
#-----------------Running index.html from react build. (Putting client into server url)---------------------
...
#-----------------GET Requests for Display pages---------------------
...
def get_exercises():
    ...
    json_exercises = [exercise.to_json() for exercise in exercises]  
...
def get_nutrition_data():
    ...
    json_nutrition_data = [nutrition.to_json() for nutrition in nutrition_data] 
...
#-----------------POST Requests for Display pages---------------------
...
def create_exercise():
    ...
    user_bodyweight = request.json.get("user_bodyweight") 
    ...
    valid_exercise_types = ['bench', 'squat', 'deadlift'] 
...
#--------------------------------Updates and Deletes to user specific requests------------------------------------
    ...
    try:
        ...
        def create_plot(title, ylabel, ydata_key):
            ...
            for exercise_type, color in exercise_colors.items():
                if exercise_data[exercise_type]['dates']: 
                    data_exists = True
            ...
            img = io.BytesIO()
            plt.savefig(img, format='png', facecolor='black') 
            img.seek(0)
